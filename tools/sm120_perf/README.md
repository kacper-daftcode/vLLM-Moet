...
| `kv_offload_probe.py --base URL --model NAME [--tokens 150000] [--evict 8]` | KV offload hit probe: one long prompt with a needle (fresh prefill), the same prompt (GPU hit), N distinct prompts of the same size (evict it from the GPU pool), the prompt again (offload hit or a fresh prefill) — wall time, answer check and the deltas of the `kv_offload_*` / prefix-cache counters per step; run the server with a small `--kv-cache-memory` so eviction is quick |
