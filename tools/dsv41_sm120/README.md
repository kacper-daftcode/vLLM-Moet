...
| `mhc_pre_norm/` | **bit-identical, not applied (slower)**: the second of the two TileLang mHC kernels per sublayer (`mhc_pre_big_fuse_with_norm`: split sums, sigmoids, 4×4 Sinkhorn, collapse of the four residual streams with the carried pre-mix, RMSNorm, draft aux; 80 launches/step, 4.7 µs each in the served graph with a [T,1,1]×96 grid) as a CUDA kernel with 32 + H/8 threads per token. `test_mhc_pre_norm_sm120.py [--bench]`: every output bit-identical to the TileLang kernel for 1–64 tokens (its reduction orders — rows xor 2,1 / columns xor 8,4 / the sum of squares over 64 threads × 16 positions and the 64-wide butterfly — are reproduced from its generated CUDA), but 4.5 µs vs TileLang's 3.5 on the RTX 5090: warp 0's 20 Sinkhorn iterations (~93 ns each) are the critical path in both, with ~1 µs of unexplained overhead in ours. Starting point for a fused sm_120 mHC (DeepGEMM's `mega_mhc` is SM100-only: tcgen05/TMEM/TMA) — superseded by `mhc_overlap/`, which takes the Sinkhorn off the critical path instead of racing it |
| `patch_vllm_mhc_overlap_sm120.py`, `mhc_overlap/` | vLLM patch (one file, `models/deepseek_v41/nvidia/ops/mhc.py`; `Dockerfile.sm120-dsv41-nightly` step 10, `-mhc` tag, **served since 2026-09-25 20:50Z**: 77 → 79–80.6 steps/s single stream, +5–7 % at eight), bit-identical: the mHC sublayer boundary off the decode critical path. The served step runs two TileLang kernels per boundary on the model stream (`mhc_fused_tilelang` post-mix + fn projection [T,12,8]×128, `mhc_pre_big_fuse_with_norm` split sums + Sinkhorn + collapse + RMSNorm [T]×96; 10.5 µs, 80 boundaries per step), but the next sublayer needs only the collapsed, normalized input — this boundary's coefficients are first read at the next boundary. Upstream's side-stream design for GB200 (`mhc_pre_delayed_overlap`, SM100 + DeepGEMM only) is enabled on sm_120 with `mhc_post_norm_sm120.cu`: `mhc_post_norm` (one CTA of H/8 threads per token: fp32 post-mix with TileLang's contraction, bf16 streams, collapse of the rounded streams with the carried pre-mix, TileLang's sum-of-squares order, RMSNorm, draft aux; 3.3 µs vs the pair's 7.4 on the RTX 5090) on the model stream and, behind the sublayer on `mhc_stream`, `mhc_proj` (the fused kernel's split partials from one CTA per split × 4 tokens, all 24 outputs, the fn slice read once per block — 8·⌈T/4⌉ CTAs instead of 96 T, so it takes little from the sublayer's kernels) followed by TileLang's `"stats"` epilogue; above 32 tokens the served path's DeepGEMM TF32 GEMM on the written streams instead. `supports_mhc_overlap` true on sm_120, `MHC_OVERLAP_MAX_TOKENS` 16 → 64 (`VLLM_MOET_MHC_OVERLAP_MAX_TOKENS`); `supports_mhc_all_reduce` true too, so upstream's `fuse_mhc_all_reduce` moves the sublayer's all-reduce out of `wo_b` / the MoE into `mhc_shifted_post_pre`, right in front of the boundary kernel (a plain all-reduce, same bits): in the captured graph the side stream's join then lands on the all-reduce node and the kernel, PDL-launched, starts 0.2 µs after it instead of 2.2 (77 × 2 µs per step). The side stream's inputs stay alive through Python references on the stream object until the next boundary instead of `record_stream`, which under capture would hold them until the capture ends (−3.2 % KV tokens). The first layer's `mhc_pre` and the Engram layers take upstream's overlap code as is. `VLLM_MOET_MHC_OVERLAP=0` restores the pair (all-reduce back in the linear layers); `VLLM_MOET_MHC_FUSE_ALLREDUCE=0`, `VLLM_MOET_MHC_PDL=0`, `VLLM_MOET_MHC_PROJ=fused\|tf32` are the sub-options. Tests (one sm_120 GPU): `mhc_overlap/test_mhc_post_norm_sm120.py [--bench] [--modes]` — streams vs `mhc_fused_tilelang` and `mhc_post_tilelang`, layer input / aux vs `mhc_pre_big_fuse_with_norm`, partials vs the fused kernel, all bit-identical for 1–64 tokens; `mhc_overlap/test_mhc_overlap_integration.py [--patch] [--bench]` — the patched `mhc_shifted_post_pre` with the side stream vs without, eagerly and inside a CUDA graph (fork / join / `record_stream` under capture), every output identical for 1–64 tokens and all three projection kinds; the bench puts one boundary in front of a compute-bound GEMM or weight-streaming GEMVs with the join after them: at 6 tokens +4.8 µs instead of the pair's +10.2. Served graph (rank-0 trace, 6 tokens): the decode step 11,152 → 10,681 µs, the attention sublayer 91.4 → 83.6 (`docs/dsv41-sm120-port.md`, "The mHC boundary off the critical path") |
...
On the vLLM-main image (`Dockerfile.sm120-dsv41-nightly`, `-mhc` tag) also the fused MoE quant+scatter, the
query-quant gate and the mHC overlap (the image is already patched; the integration test drives the patched
`mhc_shifted_post_pre` with and without the side stream, eagerly and in a CUDA graph):

```bash
docker run --rm --gpus '"device=0"' --entrypoint bash vllm-moet-sm120:dsv41-nightly-20260923-mhc -c \
  'cd /opt/vllm-moet/dsv41_sm120/moe_quant_scatter && python3 test_moe_quant_scatter_sm120.py --bench &&
   ...
   cd /opt/vllm-moet/dsv41_sm120/mhc_overlap && python3 test_mhc_post_norm_sm120.py --bench &&
   python3 test_mhc_overlap_integration.py --bench'
```

The measured-but-not-applied kernels run from the repo checkout mounted into the image (on an image without
step 10, `test_mhc_overlap_integration.py --patch` applies the mHC patch inside the throw-away container first):
...
docker run --rm --gpus '"device=0"' --entrypoint bash -v "$PWD/tools":/tools vllm-moet-sm120:dsv41-nightly-20260923-moeqs -c \
  'cd /tools/dsv41_sm120/moe_gate_topk && python3 test_moe_gate_topk_sm120.py --bench &&
   ...
   cd /tools/dsv41_sm120/mhc_overlap && python3 test_mhc_post_norm_sm120.py --bench &&
