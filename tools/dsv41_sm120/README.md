...
| `patch_deepgemm.py` | DeepGEMM host asserts: SM120 FP8 **and MXFP4** paged MQA logits on 128-row pages (the device kernels are templated on the page size; only the launchers' asserts stopped at 64). Regex anchors since 2026-09-22, so one patcher fits the 0909 image's pin (`deepseek-ai/DeepGEMM 8b1392b9`) and vLLM main's (`vllm-project/DeepGEMM e1f418c2`, `Dockerfile.sm120-dsv41-nightly`) |
