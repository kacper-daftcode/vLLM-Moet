...
INTERMEDIATE = int(os.environ.get("I", "2048"))  # 1024 TP2, 512 TP4
...
def run_forward():
    """Exercise the kernel under the same persistent-slot contract as serving."""
...
tier = moe_w2_delta.DeltaTier(
    1, E, dev, w13_bytes=2 * INTERMEDIATE * H // 2, w2_bytes=H * INTERMEDIATE // 2
)
moe_w2_delta._TIER = tier
fp13 = torch.stack(
    [pack_fp4_fragment_major(mxfp4_to_nibbles(w13_pack[e])) for e in range(E)]
)
fp2 = torch.stack(
    [pack_fp4_fragment_major(mxfp4_to_nibbles(w2_pack[e])) for e in range(E)]
)
...
cos2 = torch.nn.functional.cosine_similarity(
    got2.float().flatten(), ref2.flatten(), dim=0
).item()
print(f"DELTA mixed ({len(promoted)}/{E} promoted): max_rel={rel2:.3e} cos={cos2:.6f}")
...
tier_s = moe_w2_delta.DeltaTier(
    1, E, dev, w13_bytes=2 * INTERMEDIATE * H // 4, w2_bytes=H * INTERMEDIATE // 4
)
...
assert rf13.shape[1] == 2 * INTERMEDIATE * H // 4
assert rf2.shape[1] == H * INTERMEDIATE // 4
...
cos3 = torch.nn.functional.cosine_similarity(
    got3.float().flatten(), ref3.flatten(), dim=0
).item()
print(
    f"SPLIT mixed ({len(promoted)}/{E} promoted, slots {rf13.shape[1]}+"
