...
Quality release **`v2026.07.27-quality`** — dataset evals vs the committed **native baselines** (`bench/baselines/`, same tool/checkpoint/hardware, stock serving path). Cells: accuracy (Δpp vs native, completion-token inflation vs native). Full report: [`docs/benchmarks/v2026.07.27-quality.md`](docs/benchmarks/v2026.07.27-quality.md); process: [`bench/README.md`](bench/README.md).

| model | hardware | config | GSM8K | GPQA | GPQA think | needle | notes |
|---|---|---|---|---|---|---|---|
| deepseek-v4-flash | 2x RTX PRO 6000 TP2 | max-quality candidate (revalidation required) | **96.5%** (-0.5pp, tok +0.1%) | **73.2%** (-1.0pp, tok +28.1%) | **85.3%** (-1.0pp, tok +6.0%) | — |  |
