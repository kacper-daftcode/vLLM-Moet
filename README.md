...
Quality release **`v2026.07.31-quality`** — dataset evals vs the committed **native baselines** (`bench/baselines/`, same tool/checkpoint/hardware, stock serving path). Cells: accuracy (Δpp vs native, completion-token inflation vs native). Full report: [`docs/benchmarks/v2026.07.31-quality.md`](docs/benchmarks/v2026.07.31-quality.md); process: [`bench/README.md`](bench/README.md).

| model | hardware | config | GSM8K | GPQA | GPQA think | needle | notes |
|---|---|---|---|---|---|---|---|
| deepseek-v4-flash | 1x RTX PRO 6000 | the single unified config (resident, MTP-2, graphs) | **96.0%** (-0.5pp, tok +11.2%) | **69.7%** (-1.0pp, tok +0.6%) | — | — |  |
| deepseek-v4-flash | 1x RTX PRO 6000 | RAM-cached base, STRICT gate+repair, MTP-1, graphs | **95.5%** (-1.0pp, tok +10.0%) | **72.2%** (+1.5pp, tok +19.6%) | — | — |  |
| deepseek-v4-flash | 2x RTX PRO 6000 TP2 | max-quality candidate (revalidation required) | **96.0%** (-1.0pp, tok +1.2%) | **70.7%** (-3.5pp, tok -3.8%) | — | — |  |
