...
codebook validated on real GLM packs (`code == NIBBLE_TO_CODE[nibble]` holds
100.0000%); e2m1 magnitude 0 merges into 0.5 (the small classes have 5 members
vs 4 refinement codes — 3.6% of elements, unit err 0.5 vs the 1.0 the 2‑bit
base gives them; everything else reconstructs exactly). Desc ABI: 64 B/pair
`{a, as, base, ref, bs, c, m_rows, pad}`. Generated by `gen/gen_moe_w4s.py`,
validated by `gen/moe_w4s_check.py` (worst_rel 2.2–3.3e‑3, deterministic, PASS
on RTX PRO 6000 + RTX 5090; kernel-level cost vs `moe_w4_mm` ≈ +16–23% at
equal read bytes — the split decode is ~1.5× the ALU per word).

| cubin (`cubins-sm120/`) | SASS (`sass/`) | kernel | purpose |
|---|---|---|---|
| `moe_w4s_mm_k4096.cubin` | `moe_w4s_mm_k4096.sass` | `moe_w4s_mm` | split FP4, K=4096 (DS4 w13) |
| `moe_w4s_mm_k2048.cubin` | `moe_w4s_mm_k2048.sass` | `moe_w4s_mm` | split FP4, K=2048 (DS4 w2) |
| `moe_w4s_mm_k1024.cubin` | `moe_w4s_mm_k1024.sass` | `moe_w4s_mm` | split FP4, K=1024 (TP2 w2) |
| `moe_w4s_mm_k512.cubin` | `moe_w4s_mm_k512.sass` | `moe_w4s_mm` | split FP4, K=512 (TP4 w2), NWARP=4 |
| `moe_w4s_mm_k6144.cubin` | `moe_w4s_mm_k6144.sass` | `moe_w4s_mm` | split FP4, K=6144 (GLM‑5.x) |
| `moe_w4s_mm_k7168.cubin` | `moe_w4s_mm_k7168.sass` | `moe_w4s_mm` | split FP4, K=7168 (Kimi‑K2.x) |
